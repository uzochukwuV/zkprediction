...
import { UserBet, Prediction } from '@/types';
import { formatAmount } from '@/lib/contract';
import { Wallet, Eye, AlertCircle, CheckCircle, Clock, Lock, Trophy } from 'lucide-react';

interface MyBetsProps {
  onSelectPrediction: (id: number) => void;
}

export default function MyBets({ onSelectPrediction }: MyBetsProps) {
    ...
    return (
      <div className="bg-dark-100 rounded-2xl border border-gray-800 p-8 text-center">
        ...
        <p className="text-sm text-gray-500">
          Browse predictions and place your first bet to get started!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {bets.map((bet) => {
        const prediction = predictions.get(bet.predictionId);
        
        return (
          <div 
            key={bet.predictionId}
            onClick={() => onSelectPrediction(bet.predictionId)}
          ...
          >
            <div className="p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-xs text-gray-500 mb-1">Prediction #{bet.predictionId}</p>
                  <h4 className="text-white font-medium">
                    {prediction?.params.question || 'Loading...'}
                  </h4>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-lg font-bold ${
                      bet.choice === 0 ? 'text-primary-400' : 'text-secondary-400'
                    }`}>
                      {bet.choice === 0 ? prediction?.params.optionA : prediction?.params.optionB}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-500">Amount</p>
                    <p className="text-white font-semibold">
                      {formatAmount(bet.amount, 7)} XLM
                    </p>
                  </div>
                </div>
              </div>
            ...
            {prediction?.status === 'Resolved' && prediction.minorityOption !== null && (
              <div className={`px-4 py-3 border-t border-gray-800 ${
                bet.choice === prediction.minorityOption
                  ? 'bg-purple-500/10'
                  : 'bg-red-500/10'
              }`}>
                <div className="flex items-center justify-between text-sm">
                  <div className={bet.choice === prediction.minorityOption ? 'text-purple-400' : 'text-red-400'}>
                    {bet.choice === prediction.minorityOption 
                      ? '🎉 You were in the MINORITY!'
                      : '😔 You were in the majority'}
                  </div>
                  {bet.choice === prediction.minorityOption && !bet.claimed && (
                    <span className="text-purple-400 font-medium">Claim reward →</span>
                  )}
                </div>
              </div>
            )}
